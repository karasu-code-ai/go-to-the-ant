...
> **Branch `lang/cuda` — the CUDA edition** (one thread per agent on the GPU). This branch adds a faithful, dependency-free CUDA port
> of all six swarms in [`cuda/`](./cuda/), beside the pure-Python originals it mirrors. Each language lives on its own
