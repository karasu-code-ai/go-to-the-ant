...
> **Branch `lang/js` — the JavaScript edition** (zero-dependency, the language the browser speaks). This branch adds a faithful, dependency-free JavaScript port
> of all six swarms in [`js/`](./js/), beside the pure-Python originals it mirrors. Each language lives on its own
...
Plus the field law: **pheromone evaporates every tick.** No ant plans a route. The nest↔food path — a minimum
spanning tree in the limit (Goss et al. 1989) — *emerges* from deposit + evaporation + weighted-random following.

## What emerges (results)
```
python go_to_the_ant.py --ticks 4000 --ants 110 --seed 1
  deliveries(t): 0 0 0 1 10 14 21 21 23 27 33 37 40 47 56 64 73 81 86 89  → 95 total
python go_to_the_ant.py --ticks 5000 --ants 130 --seed 3 --wall
  deliveries(t): 0 0 0 4 5 11 15 22 30 37 43 50 54 70 79 85 93 103 108 113 → 117 total (routes the gap)
```
...
don't chase scent).** Separating read from write is what makes stigmergy work — A satisfying echo: the swarm literature's founding demo has the same failure mode
...
(wander, short memory, `p(pickup)=(k₊/(k₊+f))²`, `p(putdown)=(f/(k₋+f))²`, with k₊≈1 < k₋≈3) make like
items cluster with no ant ever comparing the whole nest. Emergent sorting: clustering climbs **0.35 → 0.92**;
...
An interactive canvas recreation of both colonies (Foraging + Brood sorting) — watch the trail and the sort
self-assemble in real time, with sliders for ants / evaporation / speed and a wall-with-a-gap toggle.
Pheromone renders as an amber heat field; searchers are cream, carriers glow. Published as an Artifact.

## The full "Go to the Ant" zoo (what the corpus annotation flattened)
Reading the whole paper (not just the INDEX note) surfaced **six** natural multi-agent systems, of which the
annotation captured only the two ant ones. The other four are each a clean recreation — the roadmap for
"evolving the swarm":
...
- **§3.3 Termites — nest building** ✅ (`termites.py` + visualizer tab) — wander toward strongest pheromone;
  deposit ∝ local density + load; scent decays so fresh piles win → scattered dabs self-concentrate into
  distinct COLUMNS (nucleate→consolidate, e.g. 3→9→6). Arches/floors are the next rung.
- **§3.4 Wasps — task differentiation** — Force + Foraging-Threshold per wasp; stochastic Fermi-function
  face-offs transfer force; brood Demand dynamics. Genetically identical wasps self-sort into Chief /
  Foragers / Nurses with no HR department (a NON-spatial emergence — the interesting one for us).
